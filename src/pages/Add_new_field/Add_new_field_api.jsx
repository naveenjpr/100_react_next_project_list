...
export default function Add_new_field_api() {
  ...
  const API_BASE_URL =
    "https://node-j-my-portfolio-mail-send.onrender.com/api/frontend/Addnewfield";

  let viewdat = () => {
    axios
      .post(`${API_BASE_URL}/view`)
      .then((result) => {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post(`${API_BASE_URL}/add`, formData)
      .then((result) => {
        setstatus(false);
        viewdat();
      })
  ...
  return (
    <>
      <SEO
        title="Add New Field API"
        description="Add new fields with API backend support"
      />

      <div
        className={`fixed w-[550px] h-auto border[solid] border-[2px] border-[red] top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] bg-white
      ...
      <div className="w-full bg-slate-400 min-h-screen">
        ...
        <div className=" max-w-[1170px] mx-auto">
          <button
            className="my-[10px] bg-blue-500 text-white px-4 py-2 float-right"
              ...
              <div key={index} className="w-full">
                <div className="overflow-hidden rounded-lg shadow-lg">
                  ...
                  <div className="flex items-center justify-between leading-tight p-2 md:p-4">
                    ...
                    <p className="text-grey-darker text-sm">{card.date}</p>
                  </div>
                  <div className="flex items-center justify-between leading-none p-2 md:p-4">
                    <p className="ml-2 text-sm">{card.author}</p>
                    <span className="hidden">Like</span>
                    <i className="fa fa-heart"></i>
                  </div>
                </div>
